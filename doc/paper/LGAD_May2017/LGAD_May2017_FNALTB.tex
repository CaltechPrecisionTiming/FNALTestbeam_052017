...
\begin{table}[!htb]
	...
	\begin{center}
		\begin{tabular}{ |c | c | c| c | }
			...
			CNM W9HG11 & -- & \textbf{-140 V, -160 V, -180 V} & -- \\ \hline
...
non-metallized area. 
%A possible explanation for this effect is a small
%difference in the rise time of the pulses that originate from the passage of
%particles in the metallized and non-metallized areas. 
We do not observe any difference in the signal rise time between the two areas.
This effect is not predicted by simulation, and cannot be
attributed to the algorithm used to time-stamp the events, since the same
behavior is observed with the CFD and CDT algorithms. Furthermore, the same
behavior is observed on all HPK sensor varieties mounted on KU board, as
presented in Sec.~\ref{sec:HPKdoping}. The CNM W9HG11 sensor does not contain
metalized
areas on its surface and we do not observe the same effect.
