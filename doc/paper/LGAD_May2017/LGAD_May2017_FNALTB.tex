...
detector. LGAD sensors manufactured by the Centro Nacional de Microelectr\`onica (CNM) and
...
be good before irradiation, with time resolution around 30-40~ps depending on
operating conditions. A ``no-response'' area between pads was
...
metallized part and 40~ps for the non-metallized area, while the HPK sensor 
...
$\eta>1.5$ for CMS, and $\eta>3.0$ for ATLAS. In order to achieve
...
LGAD sensors produced by Centro Nacional de Microelectr\`onica (CNM) and
...
the charged-particle detection efficiency, signal pulse height, and 
signal timestamp and resolution. We also compare the performance of 
...
listed in Sec.~\ref{sec:sensors}; readout boards used in the measurements are 
...
uniformity studies, studies of the gap between adjacent pixels, 
studies of the doping profile, studies of the sensor thickness,
temperature studies, and irradiation studies. A brief overview of the
...
maximum amplitude is assigned as its timestamp $t1$. For the FNAL and UCSC boards,
...
across pixels 3 and 4, and Y-axis scans across pixels 2 and 4 show
...
We present in detail the response uniformity study for the sensors produced
...
drop in efficiency is observed in the transition (gap) region between the two pixels.
A more detaied study of the gap region is given in Sec.~\ref{sec:HPK_50D_gap}.
...
(gray region in the image in Fig.~\ref{fig:HPK_Sensors}) and the non-metalized area.
A possible explanation for this small effect is a small difference in
the rise time of pulse between the metallized and non-metallized areas. 
...
mounted tested in the beam simultaneously. The HPK 50C-PIX sensor was operated 
at $-450$~V, and CNM W9HG11 sensor was operated at $-180$~V. We measure the
size of the gap region to be around 110~$\mu$m on the HPK 50C-PIX, compatible 
...
CNM W9HG11 (bottom) sensors. HPK sensor is operated at $-450$ V, and CNM sensor is operated at $-180$~V bias 
voltage. Data points in blue are those from one pixel, and data points in red are from the neighboring pixel. 
...
whether increasing the sensor thickness slightly from $50$~$\mu$m to $80$~$\mu$m
...
are relatively similar for these two different sensor thicknesses.
...
and $2\times 2$ arrays of square pixels of length 3~mm were used. Sensors with
...
sensor response in pulse height, efficiency and timing resolution were studied
and found to be good before irradiation with the exceptions of the gap between pixels
which have a width of about 70~$\mu$m for CNM sensors and 110~$\mu$m for HPK sensors. 
A small timing shift across the HPK sensor of the order 20--30~ps can
