...
timestamp, and time resolution. We also compare the uniformity of 
...
\begin{center}
  \begin{tabular}{ |c | c | c | }
    \hline
    Sensor   & Single channel dimensions &  Capacitance  \\ \hline 
    HPK 50A-PIX & $3\times 3$~$\mathrm{mm}^{2}$ & X pF \\       
    HPK 50B-PIX & $3\times 3$~$\mathrm{mm}^{2}$ & X pF \\       
    HPK 50C-PIX & $3\times 3$~$\mathrm{mm}^{2}$ & X pF \\       
    HPK 50D-PIX & $3\times 3$~$\mathrm{mm}^{2}$ & X pF \\       
    HPK 80C-PIX & $3\times 3$~$\mathrm{mm}^{2}$ & X pF \\       
    HPK 50D     & $\diameter=1.0$~$\mathrm{mm}}$  & X pF \\       
    CNM-W9HG11  & $X\times Y$~$\mathrm{mm}^{2}$ & X pF \\       
    CNM W11LGA35& $1\times 1$~$\mathrm{mm}^{2}$ & X pF \\       
    \hline
  \end{tabular}
...
at $45\%$. However the CTD algorithm at a moderate threshold does yield comparable
time resolutions. 
...
non-metalized area. A possible explanation for this effect is a small difference
...
with the CFD and CDT algorithms. Furthermore, the same behabior is observed on
...
thicknesses. This study is performed using the HPK C-PIX sensors with the same dopant concentration.
