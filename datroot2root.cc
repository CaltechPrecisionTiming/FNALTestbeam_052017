	...
	if ( !isTrigChannel ) {
	  if( drawDebugPulses ) {
	    if ( xmin[totalIndex] != 0.0 ) {
	      ...
	      RisingEdgeFitTime( pulse, index_min, 0.10, 0.50, fs, event, "linearFit_" + pulseName, true );
	    ...
	    if ( xmin[totalIndex] != 0.0 ) {
	      ...
	      RisingEdgeFitTime( pulse, index_min, 0.10, 0.50, fs, event, "linearFit_" + pulseName, false );
